...
- **`pdf.js`**'s `generateCvPdf` is the no-backend PDF fallback: it writes a vector PDF by hand (embedded Open Sans TrueType instances 400/700 as base64 in `pdf.js`, real hmtx advance widths, manual PDF byte assembly) — no library. Line-wrapping can still differ slightly from the screen (no kerning, hand-rolled layout). The WOFF2 in `fonts.css` can't be reused at runtime, so the font data is duplicated there; regeneration recipe is in the `pdf.js` comment.
