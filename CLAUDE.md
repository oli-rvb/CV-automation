...
- The Bibliothèque (`state.library`) and the CV are **independent stores**. `normalizeState` seeds the library from the CV once, keyed on the *presence* of the `library` key (not its validity) so a momentarily malformed state never re-seeds over the user's work. There is no built-in bridge in either direction — a user who wants to align them has to edit each manually, or use the separate JSON export/import features on each store.
